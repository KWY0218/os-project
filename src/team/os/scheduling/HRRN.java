...
public class HRRN implements Scheduler{
	 ...
	 * 	9. 1초를 증가한다.
	 * 	10. 코어 할당이 끝난 후 일하고 있는 코어의 누적 전력량을 갱신한다.
	 * 	11. 프로세스 동작이 끝난 후 총 전력량과 소비시간을 history에 넣는다.
	 */
	@Override
	public History schedule(List<Process> mProcessList, List<Core> mCoreList) {
		...
		while (!CPU.isTerminatedAllProcess(mProcessList)){
			...
			while(CPU.getRecommendCore(mCoreList,CPU.priorityType, -1) != -1 && !readyList.isEmpty()) {
				...
				// 5. 코어를 할당 받은 프로세스의 할당 받은 코어의 인덱스를 설정하고, waitingTime을 설정하고, working 중인 것을 명시한다. 
				readyList.get(0).setWorkingCoreIndex(coreIndex);
				readyList.get(0).setWaitingTime(currentTime-readyList.get(0).getArrivalTime());
